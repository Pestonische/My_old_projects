...
namespace BinaryTree
{
    ...
    public class Tree
    {
        ...
        public string SearchFoliage()
        {
            
            Queue<Item> wood = new Queue<Item>();
            if (root == null)
            {
                foliage = "В дереве ничего нет!";
                return foliage;
            }
            else
            {
                ...
                foliage = "Вывод всех листьев дерева:\n";
                wood.Enqueue(_root);
                while (wood.Count!=0)
                {
                    if (wood.Peek().lSon == null && wood.Peek().rSon == null)
                        foliage = foliage.Insert(foliage.Length, wood.Peek().info.ToString() + " ");
                    
                        if (wood.Peek().lSon != null)
                        {
                        wood.Enqueue(wood.Peek().lSon);
                        }
                        if (wood.Peek().rSon != null)
                        {
                        wood.Enqueue(wood.Peek().rSon);
                        }
